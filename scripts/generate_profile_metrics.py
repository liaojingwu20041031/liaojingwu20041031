#!/usr/bin/env python3
"""Render a self-hosted GitHub profile metrics SVG panel."""
...
def render_panel(user: dict, login: str, theme_name: str) -> str:
    ...
    parts = [
        ...
        f"<title>{login} GitHub engineering signal</title>",
        "<desc>Public repositories, stars, followers, yearly contributions and primary language mix.</desc>",
        "<defs>",
        ...
        svg_text(46, 39, "GITHUB / ENGINEERING SIGNAL", 12, theme["accent"], 700),
        svg_text(46, 58, f"@{login} · updated {updated} UTC", 12, theme["muted"]),
    ]

    stats = [
        ("YEAR CONTRIBUTIONS", calendar["totalContributions"]),
        ("PUBLIC REPOSITORIES", repositories["totalCount"]),
        ("PROJECT STARS", stars),
        ("FOLLOWERS", followers),
    ]
    ...
    for x, (label, value) in zip(stat_x, stats):
        ...
        parts.append(svg_text(x, 124, label, 10, theme["muted"], 600))

    parts.extend(
        [
            f'<line x1="46" y1="146" x2="914" y2="146" stroke="{theme["border"]}"/>',
            svg_text(46, 171, "52-WEEK CONTRIBUTION RHYTHM", 10, theme["muted"], 600),
        ]
    )
    ...
    for week_index, week in enumerate(weeks):
        ...
        if first_date.month != previous_month:
            month_labels.append((grid_x + week_index * (cell + gap), first_date.strftime("%b")))
    ...
    language_x = 708
    parts.append(svg_text(language_x, 171, "REPOSITORY MIX", 10, theme["muted"], 600))
